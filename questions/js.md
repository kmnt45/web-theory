...
`Обычные функции` имеют собственное значение `this`, которое определяется в момент вызова функции.
При вызове обычной функции создаётся собственный объект `arguments`. Такие функции могут использоваться с `new`.

`Стрелочные функции` не имеют собственного `this` и захватывают его из окружающего лексического контекста.
Они не создают собственный объект `arguments` и не могут использоваться с `new`.
...
`Контекст выполнения (Execution Context)` — это среда выполнения кода, которая определяет область видимости, значение `this` и порядок выполнения кода.

Основные типы контекстов выполнения:

1. Глобальный контекст выполнения:
    - Создаётся один раз при запуске программы.
    - Содержит весь глобальный код.
    - Формирует глобальное лексическое окружение.
    - В браузере `this` указывает на `window`, в Node.js — на `global`, универсально — `globalThis`.

2. Контекст выполнения функции:
    - Создаётся при каждом вызове функции.
    - Для каждого вызова — новый контекст.
    - Содержит собственные:
      - `Lexical Environment`
      - значение `this`
      - параметры функции и `arguments` (кроме стрелочных функций).

3. Контекст выполнения модуля
    - Используется для ES-модулей.
...
1. Фаза создания (Creation Phase):
    - Формируется `Lexical Environment` — это структура, которая состоит из:
      - `Environment Record` — хранит объявления переменных и функций;
      - `Outer Lexical Environment Reference` — ссылка на внешнее лексическое окружение;

   Что происходит на этой фазе:

    - Обрабатываются объявления:
      - `function` → полностью инициализируются;
      - `var` → создаются и инициализируются `undefined`;
      - `let / const` → создаются, но находятся в `TDZ`;
    - Запоминаются параметры функции;
    - Создаётся `arguments` (не у стрелочных функций);
    - Формируется `Scope Chain` через `Outer Lexical Environment`;
    - Определяется значение `this`:
      - глобальный контекст → глобальный объект;
      - функция → зависит от способа вызова;
...
2. Фаза выполнения (Execution Phase):
    ...
    - При вызове функций создаются новые `execution contexts`, которые кладутся в `call stack`

Связь с `Scope Chain`:

- `Scope Chain` строится из цепочки `Lexical Environment`
- При поиске переменной:
  - текущий Environment Record
...
`Модульная` — только в ES-модулях. Всё внутри файла изолировано. Нет автоматического попадания в глобальный scope.
...
## 27. Округление Math.floor(), Math.round(), Math.ceil() и Math.trunc()?
...
`Map` — коллекция пар `ключ-значение`, где ключи могут быть любого типа. Порядок элементов сохраняется.
...
- `entries()` – возвращает итерируемый объект по парам вида `[ключ, значение]`, этот вариант используется
...
- `add(value)` – добавляет значение (если оно уже есть, то ничего не делает), возвращает тот же объект
  Set.
- `delete(value)` – удаляет значение, возвращает `true`, если `value` было в множестве на момент вызова,
  иначе `false`.
...
- `entries()` – возвращает перебираемый объект для пар вида `[значение, значение]`, присутствует для
  обратной совместимости с Map.

`WeakMap и WeakSet` — аналоги `Map и Set`, но работают только с объектами и хранят их по слабым ссылкам, не препятствуя сборке мусора.

- `Map / Set` → сильные ссылки, объекты удерживаются в памяти

- `WeakMap / WeakSet` → слабые ссылки, объекты удаляются GC при отсутствии других ссылок
...
`Babel` — это транспилер JavaScript, который позволяет писать современный код (ES6+) и преобразует его в старый стандарт (ES5), чтобы он работал в любых браузерах.

- Он транспилирует синтаксис (стрелочные функции, `let/const`, классы и др.).

- Может подключать полифилы для новых API (`Promise`, `fetch`, `Array.from`) в старых браузерах.

- Обычно используется вместе со сборщиками вроде Webpack, Parcel, Vite.
...
## 60. Можно ли реализовать реактивность, подобную React, с помощью чистого JS?

Да, можно реализовать реактивность без React, с помощью:

- `Observer pattern` (наблюдаемые объекты)

- `Proxy` или `Object.defineProperty` для отслеживания изменений данных

- При изменении состояния автоматически обновляется DOM

Пример мини-реактивности с Proxy:
...
// Изменение данных
state.count = 1; // автоматически обновит DOM
```
...
## 61. Количество потоков в JS? Какая адаптация к однопоточности, существует в JS для эмуляции многопоточности? Есть ли способ, использовать истинную многопоточность в JS (реальная работа с несколькими потоками процессора)?

`JavaScript в браузере и Node.js однопоточный` — есть один главный поток выполнения (main thread), который обрабатывает код, UI и события.
Однопоточность означает, что код выполняется по очереди, блокируя другие задачи, если долго выполняется.

В JavaScript, несмотря на однопоточность, используется асинхронность через Event Loop для эмуляции многопоточности.
Асинхронные операции — таймеры (`setTimeout`, `setInterval`), сетевые запросы (`fetch`), события DOM — ставятся в очередь и выполняются после завершения текущего стека вызовов.
Механизмы работы с асинхронным кодом включают `Callbacks`, `Promises` и `async/await`, что позволяет JS эффективно обрабатывать несколько задач "параллельно", не блокируя главный поток.

Реальную многопоточность в JS можно использовать через:

- `Web Workers (браузер)` — отдельные JS-файлы выполняются в собственных потоках.

- `Worker Threads (Node.js)` — отдельные потоки для тяжёлых вычислений.

Особенности:

- Потоки не разделяют память напрямую, общение происходит через сообщения (`postMessage`).

- Используется для тяжёлых вычислений, чтобы не блокировать главный поток.
...
## 23.  Что такое Browser History? Можем ли мы взаимодействовать с историей браузера через JS? Где применяется подобное взаимодействие?

`Browser History` — это список URL, которые пользователь посещал в текущей вкладке/окне. Браузер хранит её локально и позволяет пользователю переходить вперёд и назад (кнопки "Назад"/"Вперед").

Можно ли взаимодействовать через JavaScript:

Да, через объект `window.history` можно:
...
- `history.pushState(state, title, url)` — добавить новую запись в историю без перезагрузки страницы

- `history.replaceState(state, title, url)` — заменить текущую запись истории

Где применяется:

- Одностраничные приложения (SPA): для управления URL без перезагрузки страницы (`pushState` / `replaceState`)

- Навигация вперёд/назад: реализация кнопок "Назад" и "Вперед" внутри SPA

- Динамические изменения URL: например, фильтры, пагинация, вкладки, которые меняют адрес, но не перезагружают страницу

```javascript
// Добавляем запись в историю без перезагрузки
history.pushState({page: 2}, "Title", "/page2");

// Обработка события изменения истории
window.onpopstate = function(event) {
  console.log("Текущая страница:", event.state);
};
```

💡 Важно:

- Через JS нельзя получать полный список истории браузера по соображениям безопасности и приватности.

- Можно работать только с историей текущего окна/вкладки и управлять своими записями (`pushState` / `replaceState`).
